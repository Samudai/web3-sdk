...
import './npm/@ensdomains/ens-contracts/contracts/registry/ENS.sol';
import './npm/@ensdomains/ens-contracts/contracts/wrapper/INameWrapper.sol';
import './npm/@ensdomains/ens-contracts/contracts/wrapper/NameWrapper.sol';

import './github/ensdomains/ens-contracts/contracts/resolvers/profiles/ContentHashResolver.sol';

import './npm/@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol';
import './npm/@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol';
import './npm/@openzeppelin/contracts-upgradeable/token/ERC1155/IERC1155ReceiverUpgradeable.sol';
import './npm/@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol';
  ...
  OwnableUpgradeable
{
  ...
  function getData(
    bytes32 parentNode
  ) public view returns (address owner, uint32 fuses, uint64 maxExpiry) {
  ...
  function transferSubdomainOwnership(
    bytes32 subNode,
    address subdomainOwner
  ) public onlyTokenOwner(subNode) {
  ...
  function createSubdomainWithContentHash(
    bytes32 parentNode,
    string memory label,
    bytes calldata hash,
    uint32 fuses,
    address resolver_add,
    address subdomain_owner
...
contract ClaimSubdomainV2 is ClaimSubdomain {
  ...
  function createSubdomainWithContentHashV2(
    bytes32 parentNode,
    string memory label,
    bytes calldata hash,
    uint32 fuses,
